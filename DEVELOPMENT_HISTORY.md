...
| :--- | :--- | :--- | :--- | :--- |
| 2026-09-07 13:44 | `Local Pending` | Antigravity | **Fix:** Resolved Pooja keyword matching false-positive bug by enforcing regex word boundary checks (`(?<!\w).*(?!\w)`) for single & multi-word keywords in `filter_by_keywords.py` & `tasks.py`, eliminating bogus substring matches (e.g. `ge` matching `league` or `ct` matching `products`). | Completed & Local |
