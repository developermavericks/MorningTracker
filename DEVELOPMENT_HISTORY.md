...
| 2026-09-11 15:20 | `Pending` | Antigravity | **Fix:** Added `strict_competitor_filter` and `strict_section_matching` column migration queries to `init_db()` (async database initializer in `backend/db/database.py`), resolving Railway production HTTP 500 error on `GET /api/clients/` caused by missing PostgreSQL columns. | Completed & Local |
