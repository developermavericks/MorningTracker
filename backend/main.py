...
from routers import scrape, articles, diagnostics, brands, auth, admin, clients, heavy_automation, robust_automation, historical_automation, feed
...
# We set same_site="lax" and https_only=True to allow secure cookies that bypass browser third-party control blocks.
...
# Proxy Headers (Outermost, so it runs first on requests to rewrite scheme/IPs)
app.add_middleware(
...
async def start_scheduler():
    """Start API background scheduler to trigger periodic Celery tasks."""
    ...
    def trigger_checks():
        try:
            logger.info("APScheduler: Triggering periodic Celery tasks...")
            celery_app.send_task("scraper.tasks.check_client_schedules")
            celery_app.send_task("scraper.tasks.complete_stale_jobs")
        except Exception as e:
            logger.error(f"APScheduler failed to send periodic tasks: {e}")

    # Run every 5 minutes, starting immediately on startup
    scheduler.add_job(trigger_checks, 'interval', minutes=5, next_run_time=datetime.now())
    scheduler.start()
    logger.info("APScheduler started successfully (Interval: 5 minutes).")
...
api_router.include_router(heavy_automation.router)
api_router.include_router(robust_automation.router)
api_router.include_router(historical_automation.router)
api_router.include_router(feed.router, prefix="", tags=["feed"])
